...
class RNNSLU(object):
    ...
    def train(self, x, y, window_size, learning_rate):
        ...
        words = list([numpy.asarray(x).astype('int32') for x in cwords])
        labels = y
...
def main(param=None):
    ...
    idx2label = dict((k, v) for v, k in list(dic['labels2idx'].items()))
    idx2word = dict((k, v) for v, k in list(dic['words2idx'].items()))
